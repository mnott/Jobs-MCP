...
const server = new McpServer(
  ...
  },
  {
    instructions: [
      ...
      "### Templates",
      "",
      "- **LinkedIn** (HTTP) — deterministic regex extraction, no browser.",
      "- **Glassdoor** (Playwright) — headless browser, only if the optional",
      "  `playwright` dep is installed (`npm install playwright && npx playwright install chromium`).",
      "",
      "Taleo, Avature, JobUp, Workday, Teamtailor, etc. land next.",
    ].join("\n"),
  },
);
